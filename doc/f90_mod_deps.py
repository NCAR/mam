...
def get_deps_and_mods(filename, opts):
    ...
    for line in f:
        ...
        if match:
            sys.stderr.write("comment line: %s\n" % line)
            line = match.group(1)
            sys.stderr.write("trimmed comment line: %s\n" % line)
	...
	if within_use_statement:
            match = only_re.search(rest_line)
            if match:
                sys.stderr.write("only line: %s\n" % rest_line)
                rest_line = match.group(1)
                sys.stderr.write("trimmed only line: %s\n" % rest_line)
