...
!
!> \file
!> The mam_mode_shortwave_optics_lookup module

!> The mode_shortwave_optics_lookup_t type and related functions
module mam_mode_shortwave_optics_lookup
  ...
  public :: mode_shortwave_optics_lookup_t

  type :: mode_shortwave_optics_lookup_t
    private
    ...
    real(kind=musica_dk), allocatable :: extinction_(:,:,:,:)
    real(kind=musica_dk), allocatable :: asymmetry_parameter_( :,:,:,:)
    !> @}
  ...
  end type mode_shortwave_optics_lookup_t

  interface mode_shortwave_optics_lookup_t
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  function constructor( config ) result( new_lookup )
    ...
    type(mode_shortwave_optics_lookup_t)                :: new_lookup
    class(config_t),                      intent(inout) :: config

    character(len=*), parameter :: my_name =                                  &
        "mode_shortwave_optics_lookup_t constructor"
    ...
    call get_file_data( file_path, variable_name, new_lookup%absorption_,     &
                        my_name )
    call config%get( "specific extinction lookup", table_config, my_name )
    call table_config%get( "variable name", variable_name, my_name )
    call get_file_data( file_path, variable_name, new_lookup%extinction_,     &
                        my_name )
    call config%get( "asymmetry factor lookup", table_config, my_name )
    call table_config%get( "variable name", variable_name, my_name )
    call get_file_data( file_path, variable_name,                             &
                        new_lookup%asymmetry_parameter_, my_name )
    new_lookup%number_of_wavelength_bands_ = size( new_lookup%absorption_, 4 )
    call assert_msg( 222130205, new_lookup%number_of_wavelength_bands_ .eq.   &
                                size( new_lookup%extinction_, 4 ),            &
                     "Dimension mismatch in shortwave extinction lookup data" )
    call assert_msg( 550462160, new_lookup%number_of_wavelength_bands_ .eq.   &
                                size( new_lookup%asymmetry_parameter_, 4 ),   &
                     "Dimension mismatch in shortwave asymmetry parameter "// &
                     "lookup data" )
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  pure subroutine get_optics( this, refractive_indices, absorption,           &
      extinction, asymmetry_parameter )

    class(mode_shortwave_optics_lookup_t), intent(in) :: this
    ...
    real(kind=musica_dk), optional, intent(out) :: asymmetry_parameter(:,:)
    !> @}
      ...
      if( present( extinction ) ) then
        call this%axes_( i_band )%interpolate(                                &
            this%extinction_( :,                                              &
                              ...
                              indices(2):indices(2)+1,                        &
                              i_band ),                                       &
            residuals, extinction(:, i_band ) )
      end if
      if( present( asymmetry_parameter ) ) then
        call this%axes_( i_band )%interpolate(                                &
            this%asymmetry_parameter_( :,                                     &
                                       indices(1):indices(1)+1,               &
                                       indices(2):indices(2)+1,               &
                                       i_band ),                              &
            residuals, asymmetry_parameter(:, i_band ) )
      end if
    end do
...
end module mam_mode_shortwave_optics_lookup
